...
namespace UcenjeCS.E18KonzolnaAplikacija
{
    internal class ObradaPolaznik
    {
        ...
        private void PromjenaPodatakaPolaznika()
        {
            PrikaziPolaznike();
            var odabir = Polaznici[Pomocno.UcitajRasponBroja("Odaberi redni broj polaznika za promjenu", 1, Polaznici.Count) - 1];
            odabir.Sifra = Pomocno.UcitajRasponBroja("Unesi šifru polaznika", 1, int.MaxValue);
            odabir.Ime = Pomocno.UcitajString("Unesi ime polaznika", 50, true);
            odabir.Prezime = Pomocno.UcitajString("Unesi prezime polaznika", 50, true);
            odabir.Email = Pomocno.UcitajString("Unesi email polaznika", 50, true);
            odabir.OIB = Pomocno.UcitajString("Unesi OIB polaznika", 50, true);
        }
